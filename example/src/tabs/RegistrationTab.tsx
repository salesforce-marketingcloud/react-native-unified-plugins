...
export default function RegistrationTab({ sfmc, mc }: Props) {
    ...
    async function loadIdentity() {
        try { setProfileId((await sfmc.getProfileId?.()) ?? ''); } catch { setProfileId(''); }
        try { setPartyName((await sfmc.getPartyIdentificationName?.()) ?? ''); } catch { setPartyName(''); }
    ...
    async function saveProfileId() {
        if (!profileId.trim()) { Alert.alert('Enter a profile ID'); return; }
    ...
    async function saveAttribute() {
        if (!attrKey.trim()) { Alert.alert('Enter an attribute key'); return; }
    ...
    async function addTag() {
        if (!tag.trim()) { Alert.alert('Enter a tag'); return; }
        if (savedTags.includes(tag.trim())) { Alert.alert('Tag already added'); return; }
    ...
    return (
        ...
        >
            ...
            <PrimaryButton title="Save Party Identification" onPress={savePartyIdentification} />
            ...
            <Card>
                ...
                {savedAttrs.map(a => (
                    <View key={a.key} style={s.savedRow}>
                        ...
                        <TouchableOpacity onPress={() => clearAttribute(a.key)} style={s.clearBtn}>
