...
export default function HomeTab({
  ...
  const [lastKnownLocation, setLastKnownLocation] = useState<string | null>(
    null,
  );
  ...
  async function ensureNotificationPermission(): Promise<boolean> {
    ...
    const result = await PermissionsAndroid.request(perm, {
      ...
      message:
        "Enable notifications to receive push messages from Marketing Cloud.",
  ...
  function onMcAnalyticsToggle(v: boolean) {
    setMcAnalytics(v);
    if (v) mc.enableAnalytics();
    else mc.disableAnalytics();
  }

  function onMamAnalyticsToggle(v: boolean) {
    setMamAnalytics(v);
    if (v) mam.enableAnalytics();
    else mam.disableAnalytics();
  }
  ...
  function sendEvent() {
    ...
    if (attrKey.trim() && attrValue.trim())
      attrs[attrKey.trim()] = attrValue.trim();
    const event = {
      objType: "CustomEvent" as const,
      name: eventName.trim(),
      attributes: attrs,
    };
    if (sendImmediate) {
  ...
  async function refreshLastKnownLocation() {
    ...
    if (loc.latitude != null && loc.longitude != null) {
      setLastKnownLocation(`${loc.latitude}, ${loc.longitude}`);
    } else {
    ...
    >
      ...
      <Card>
        ...
        <Row
          ...
          onPress={
            mcDeviceId
              ? () => copyToClipboard("MCE Device ID", mcDeviceId)
              : undefined
          }
        />
        <Row
          ...
          onPress={
            mamDeviceId
              ? () => copyToClipboard("MAM Device ID", mamDeviceId)
              : undefined
          }
        />
      </Card>
      ...
      <Card>
        <View style={s.switchRow}>
          ...
          <Switch
            ...
            accessibilityLabel="Push Enabled"
          />
        </View>
      </Card>
      ...
      <Card>
        <View style={s.switchRow}>
          ...
          <Switch
            value={mcAnalytics}
            onValueChange={onMcAnalyticsToggle}
            accessibilityLabel="MCE Analytics"
          />
        </View>
        <View style={[s.switchRow, s.noBorder]}>
          ...
          <Switch
            value={mamAnalytics}
            onValueChange={onMamAnalyticsToggle}
            accessibilityLabel="MAM Analytics"
          />
        </View>
      </Card>
      ...
      <Card>
        ...
        <View style={[s.switchRow, s.noBorder]}>
          <Text style={s.switchLabel}>
            Proximity Messaging{Platform.OS === "ios" ? " (Android only)" : ""}
          </Text>
          <Switch
      ...
      <PrimaryButton
        ...
        onPress={() => {
          sfmc.flush();
          Alert.alert("Flushed", "Event queue flushed.");
        }}
      />
        ...
        <View style={[s.switchRow, s.noBorder]}>
          <View style={s.switchLabelColumn}>
            ...
            <Text style={s.switchSubLabel}>
              Run a custom code rule per message instead of showing
              automatically.
            </Text>
          </View>
          <Switch
            value={iamDecisionMode}
            onValueChange={onIamDecisionModeToggle}
            accessibilityLabel="App decides display"
          />
        </View>
      </Card>
      ...
      <PrimaryButton
        title="Set Status Bar Color (Android)"
        onPress={applyIamStatusBarColor}
      />
      ...
      >
        <View style={s.modal}>
          ...
          <View style={s.switchRow}>
            ...
            <Switch
              value={sendImmediate}
              onValueChange={setSendImmediate}
              accessibilityLabel="Send Immediate"
            />
          </View>
