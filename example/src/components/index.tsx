...
export function Row({ label, value, onPress }: RowProps) {
    return (
        ...
        >
            ...
            {value != null && <Text style={s.rowValue} numberOfLines={1}>{value}</Text>}
        </TouchableOpacity>
    );
}

export function MultilineRow({ label, value }: { label: string; value?: string | null }) {
    return (
        <View style={s.row}>
            <Text style={s.rowLabel}>{label}</Text>
...
export function PrimaryButton({ title, onPress, destructive, disabled }: PrimaryButtonProps) {
    return (
        <TouchableOpacity
            style={[s.button, destructive && s.buttonDestructive, disabled && s.buttonDisabled]}
            onPress={onPress}
            disabled={disabled}
            activeOpacity={0.7}
        >
            <Text style={[s.buttonText, destructive && s.buttonTextDestructive]}>{title}</Text>
        </TouchableOpacity>
    );
}
