...
function AppInner() {
  const [sdks, setSdks] = useState<SDKState>({
    sfmc: null,
    push: null,
    iam: null,
    mc: null,
  ...
  const showInboxActions =
    activeTab === "inbox" && inboxActionsReady && inboxActionsRef.current;
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.navBar}>
        ...
        {showInboxActions && (
          <View style={s.navRight}>
            <TouchableOpacity
              onPress={() => inboxActionsRef.current?.markAllRead()}
            >
              <Text style={s.navIcon}>✓</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => inboxActionsRef.current?.deleteAll()}
            >
      ...
      <View style={s.content}>
        ...
        {activeTab === "debug" && sdks.sfmc && <DebugTab sfmc={sdks.sfmc} />}
        {/* Fallback if primary module failed */}
        {activeTab === "home" &&
          (!sdks.sfmc || !sdks.push || !sdks.mc || !sdks.mam || !sdks.iam) && (
            <View style={s.unavailable}>
              <Text style={s.unavailableText}>
                One or more SDK modules failed to initialise. See warnings
                above.
              </Text>
            </View>
          )}
