...
|---|---|---|
| [`@salesforce-mc/react-native-sfmc-core`](./packages/sfmc-core) | Foundation: identity, custom attributes, structured events, logging | `setProfileId`, `setAttribute(s)`, `track`, `setLogging`, `getSdkState` |
| [`@salesforce-mc/react-native-push`](./packages/push) | Push registration & notifications | `enablePush`, `disablePush`, `getSystemToken`, `areNotificationsEnabled` |
| [`@salesforce-mc/react-native-iam`](./packages/iam) | In-App Messaging lifecycle events | event-driven (`getEmitter()`) |
| [`@salesforce-mc/react-native-marketingcloudsdk`](./packages/marketingcloudsdk) | MarketingCloud Engagement: inbox, tags, attributes, analytics | `getAllMessages`, `markMessageRead`, `addTag(s)`, `enableAnalytics`, `getContactKey` |
| [`@salesforce-mc/react-native-mobileappmessaging`](./packages/mobileappmessaging) | MobileAppMessaging: modern lightweight product | product-specific |
...
- **Native event emission** — modules expose `getEmitter(): NativeEventEmitter` for streaming events (push token refresh, IAM lifecycle, etc.).
...
await sdk.setProfileId('user-123');
await sdk.setAttribute('plan', 'pro');
await sdk.track({ name: 'purchase', amount: 49.99 });

// Use the engagement product
const mc = await MarketingCloudSdkModule.requestSdk();
const unread = await mc.getUnreadMessages();
await mc.markMessageRead(unread[0].id);
```
...
| Java | 17 |
| Ruby | for CocoaPods (see `example/.ruby-version`) |
